...
class BasePage:
    ...
    def type(self, locator, value):
        if str(locator).endswith("_XPATH"):
            self.find_element(AppiumBy.XPATH, configReader.readConfig("locators", locator)).send_keys(value)
        elif str(locator).endswith("_ACCESSIBILITYID"):
    ...
    def getText(self, locator):
        if str(locator).endswith("_XPATH"):
            text = self.find_element(AppiumBy.XPATH, configReader.readConfig("locators", locator)).text
        elif str(locator).endswith("_ACCESSIBILITYID"):
